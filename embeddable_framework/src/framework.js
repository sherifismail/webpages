...
window.Framework = {
    config: {
        ...
        clientIds: {
            'mypurecloud.com': '',
            'mypurecloud.ie': '4a76f883-7c1a-4572-8651-dcaf17d9d124',
            'mypurecloud.com.au': '',
            'mypurecloud.jp': '',
            'mypurecloud.de': ''
        },
        customInteractionAttributes: ['PT_URLPop', 'PT_SearchValue', 'PT_TransferContext', 'CustomerName','TemplateID'],
        settings: {
    ...
    initialSetup: function () {
        //console.log(`New Token Acquired:${window.PureCloud.User.getAuthToken()}`);
        window.PureCloud.subscribe([
            {
                type: 'Interaction', 
                callback: function (category, interaction) {
                    try {
                        console.log(`token:${window.PureCloud.User.getAuthToken()}`);
                    }catch(error) {
                        console.log(error);
                    }
        ...
        window.PureCloud.User.getAuthToken((data)=>{
            window.parent.postMessage(JSON.stringify({type:"getAuthToken", data:data}) , "*");    
        });
