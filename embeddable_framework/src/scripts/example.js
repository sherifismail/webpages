document.addEventListener('DOMContentLoaded',function(){
    ...
    window.addEventListener("message", function(event) {
        ...
        if(message){
            if(message.type == "screenPop"){
                ...
                console.log(`BearerTokenReceived:${JSON.stringify(message)}`);
            }
        }
    });
    ...
    function clickToDial() {
        ...
        /**
         * Search the customer contact based on ACcount IT
         * FInd the encrypted contact
         * Send the dial request to genesys
         */

        document.getElementById("softphone").contentWindow.postMessage(JSON.stringify({
            type: 'clickToDial',
            data: { number: '+971528674430', autoPlace: true }
        }), "*");
    }
